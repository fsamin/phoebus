...
- **Scroll-based lesson progress:** For lesson steps, `in_progress` is set only after the learner scrolls through ≥75% of the content (not on page open, unlike exercises). Scroll detection uses a client-side `IntersectionObserver` — no new API endpoints are needed. When the threshold is reached, the frontend calls the existing `POST /api/progress` with status `in_progress`. Lessons use a unified "Complete & Continue" button (disabled until 75% scroll) that marks the step as completed and navigates forward. On the last step, a congratulations modal is shown before redirecting to the path overview.
...
- **`progress`** — tracks step-level completion status. `status` enum: `not_started`, `in_progress`, `completed`. `in_progress` is a distinct third state displayed in all analytics (icon: ⏳ ClockCircleOutlined). For lessons, `in_progress` is set via client-side scroll tracking (≥75% threshold) rather than on page open
